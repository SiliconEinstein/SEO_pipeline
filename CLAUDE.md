...
def run(config, output_dir):
    ...
    #   --top 模式：加载全部排名页面 → 过滤 → 取前 N（保证凑满请求数量）
...
- **`_compute_trends()`**（始终运行）：只要 `daily_pages_*.csv` 存在就运行。在唯一路径上调用一次 `discover_subtypes()`，再 merge 回完整 df，按日期聚合为 overall / per-subtype / _optimized_ 三个层级。
