...
1. 在 `config.yaml` 中设置 `lance.enabled: true`，并配置 `lance.tos.*`。  
2. 在 `.env` 中配置 `TOS_ACCESS_KEY` / `TOS_SECRET_KEY`。  
3. 首次执行 `uv run python main.py init-lance` 创建表结构。

去重选择规则（optimize）：

- 跳过近 `lance.skip_recent_days` 天内已优化的 URL。  
- `--top N`：先去重，再从后续排名页回填，目标是尽量保留 N 个。  
- `--range A-B`：仅在该范围内去重，不会跨范围回填，因此实际数量可能少于范围长度。  
- 若去重后没有可优化 URL，本次 optimize 返回 `skipped`（正常结束，不报错）。
...
# 编辑 config.yaml：
#   seo.include_subtypes: ["sciencepedia/feynman", "sciencepedia/feynman/keyword"]
#   seo.subtype_page_types: {"sciencepedia/feynman": "course_article", "sciencepedia/feynman/keyword": "keyword"}
#   （以 analyze 报告中的子类型标签为准）
...
首次部署到云服务器后，建议先手动跑通一次（以 `/opt/SEO_pipeline` 为例）：
...
3. 执行上传钩子 `scripts/upload_optimized.py`。  
4. 仅当上传钩子成功（exit 0）后，才把 `output/` 重命名为 `output_archive/output_YYYYmmdd_HHMMSS/`，并新建空 `output/`。

这样即可按运行时间区分每次产物，不会混淆历史结果。
...
```cron
0 3 */14 * * cd /opt/SEO_pipeline && /usr/bin/flock -n /tmp/seo-pipeline.lock /bin/bash scripts/run_pipeline_scheduled.sh
```

> 注：`cron` 的 `*/14` 基于“每月日期”触发，不是严格固定 14 天间隔。

当前脚本固定 Top 100；如果你要改规模，直接编辑脚本中的 `TOP_N`。`evaluate` 仍建议独立按周/月运行。
...
参数组合建议可参考本地文件 `docs/optimize_param_profiles.md`（默认 Top 30，也可按需用 `--top` 提升到 100/500 等）。
...
按 `priority_score = impressions × (1 - CTR)` 计算每个页面的优化价值，按 `seo.include_subtypes` 筛选后输出排序列表。高展示低点击率的页面排在前面。默认会优先保留有零点击查询词的页面（这些页面有搜索展示但没有点击，查询词数据为 LLM 优化提供关键词上下文）；若 zero-click 数据缺失或为空，则回退为按页面级数据全量排序。
...
**优化去重与历史写入规则：** 见上文“优化历史与去重规则（Lance）”。
...
- **趋势分析**（`daily_pages_*.csv` 存在且非空时运行）：按子类型分板块的逐日点击、展示、加权 CTR、平均排名
...
| `optimize.top` | `30` | 默认处理 Top N 页面（可被 `--top` 覆盖） |
